...
while IFS='' read -r line || [[ -n "$line" ]]; do
    if [ "$line" == "/* PASTE EXTERNAL CSS HERE */" ]; then
        ...
        cat ${files} | yui-compressor --line-break 300 --verbose --type css --charset utf8 >> "$out"
        printf "\n" >> "${out}"
        continue
    fi
    ...
    if [ "$line" == "<!-- PASTE EXTERNAL JS HERE -->" ]; then
        ...
        cat ${files} | yui-compressor --line-break 300 --verbose --type js --charset utf8 -v >> "$out"
        printf "\n</script>\n" >> "$out"
        continue
    fi
        ...
        do
            ...
            if [ -z "$file" ]; then
                ...
                if [ -f "$file" ]; then
                    ...
                    printf "<audio id='%s' src='data:${mimetype};base64,%s'></audio>\n" "${id}" "${b64}" >> "$out"
                else
